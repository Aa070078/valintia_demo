...
export function VolumeCard({
  ...
  return (
    <button
      ...
      className={cn(
        "group relative flex flex-col overflow-hidden rounded-2xl border text-left transition-all duration-300 cursor-pointer select-none",
    ...
    >
      ...
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-[#EDE6DB]">
        ...
        <div className="absolute bottom-2.5 left-2.5 z-10 rounded bg-[#1C1917]/85 px-2 py-0.5 backdrop-blur-xs shadow-2xs">
        ...
        {selected && (
          <div className="absolute top-2.5 right-2.5 z-10 flex h-6 w-6 items-center justify-center rounded-full bg-white text-[#1C1917] shadow-md ring-2 ring-white/80 animate-in fade-in zoom-in-75 duration-200">
      ...
      <div className="flex flex-1 flex-col justify-between p-3.5 sm:p-4 bg-[#FAF6F0] dark:bg-[#1A1A1E]">
        ...
        <div className="mt-3 flex items-center justify-between border-t border-[#E8DFD3] dark:border-[#2C2C32] pt-2 text-[10px] text-[#78716C] dark:text-[#989692]">
          ...
          <ArrowUpRight
            size={13}
            weight="bold"
            className="shrink-0 transition-transform duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 text-[#1C1917] dark:text-[#FAF7F2]"
          />
        </div>
      </div>
    </button>
